...
[![.NET](https://img.shields.io/github/actions/workflow/status/JKorf/BingX.Net/dotnet.yml?style=for-the-badge)](https://github.com/JKorf/BingX.Net/actions/workflows/dotnet.yml) ![License](https://img.shields.io/github/license/JKorf/BingX.Net?style=for-the-badge) ![Since](https://img.shields.io/badge/since-2024-brightgreen?style=for-the-badge)

[![Docs](https://img.shields.io/badge/Docs-BingX.Net-1b7f50?style=for-the-badge)](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=BingX.Net)
...
The [BingX.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=BingX.Net) is the main resource for installing, configuring, and using the library.

| Resource | Description |
|--|--|
| [Client guide](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=BingX.Net) | Installation, REST and WebSocket clients, authentication, dependency injection, error handling, and advanced features |
| [Examples](https://cryptoexchange.jkorf.dev/docs/exchange-clients/examples?library=BingX.Net) | Common REST and WebSocket operations |
| [API reference](https://cryptoexchange.jkorf.dev/docs/exchange-clients/reference?library=BingX.Net) | Client interfaces, methods, and properties |
...
For more examples and explanations, continue with the [BingX.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=BingX.Net) or browse the [compilable repository examples](https://github.com/JKorf/BingX.Net/tree/main/Examples).
...
V2 uses a strict interface per capability. The `SharedApi` property on each API surface therefore exposes only the operations or subscriptions that surface actually supports. `IBingXSharedApiClient` groups the exchange's Shared API surfaces for dependency injection and runtime capability lookup.
...
using var restClient = new BingXRestClient();
...
The request and response models belong to `CryptoExchange.Net.SharedApis`, so the same operation can accept another exchange's `IGetTickerRest` implementation. When using dependency injection, inject `IBingXSharedApiClient` to access all of the exchange's Shared API surfaces or inject a capability such as `IGetTickerRest` directly. Use `GetCapability` on the aggregate when the operation, transport, or trading mode is selected at runtime.

## AI documentation
For AI coding assistants and quick onboarding:
* [AGENTS.md](AGENTS.md) skill-style instructions for BingX.Net usage
...
## Release notes
* Version 4.6.0 - 24 Sep 2026
    * Updated CryptoExchange.Net to v13.0.0
    * Shared APIs
      * Added SharedApi V2 support
      * Added `.SharedApi` property to clients
      * Added `(I)BingXSharedApiClient` containing all V2 SharedApi references
      * Added `SharedApi` to `BingXOptions` to provide SharedApi related options
    ...
    * Added Expired value to OrderStatus enum
    * Fixed PositionId deserialization on BingXFuturesOrderDetails model

* Version 4.5.0 - 21 Aug 2026
    * Updated to CryptoExchange.Net v12.5.0
    * Added MakerFeePercentage, TakerFeePercentage to SharedFuturesSymbol
...
* Version 4.1.0 - 09 Jul 2026
    ...
    * Updated RealizedProfit and FrozenMargin mapping on BingXFuturesBalance model
    * Updated documentation links
    * Renamed orderListId to orderId for SpotApi.Trading.GetOcoOrderAsync endpoint
...
* Version 4.0.0 - 29 Jun 2026
    ...
    * Added setter to BingXExchange.RateLimiter to allow custom rate limit settings
    * Various small performance improvements
    * Fixed websocket connection attempts counting towards rate limit even when server could not be reached
    * Fixed Futures order update ClientOrderId deserialization
