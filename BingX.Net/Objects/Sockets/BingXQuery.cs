...
namespace BingX.Net.Objects.Sockets
{
    internal class BingXQuery : Query<BingXSocketResponse>
    {
        ...
        {
            _client = client;
            MessageRouter = MessageRouter.CreateForQuery<BingXSocketResponse>(request.Id, HandleMessage);
        }
        ...
        {
            if (message.Code != 0)
                return CallResult.Fail<BingXSocketResponse>(new ServerError(message.Code.ToString(), _client.GetErrorInfo(message.Code, message.Message!)), originalData);
